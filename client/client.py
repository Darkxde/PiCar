...
def video_show():
    ...
    player = subprocess.Popen(cmdline, stdout=subprocess.PIPE, stdin = subprocess.PIPE, stderr=subprocess.PIPE)

    server_socket = socket(AF_INET, SOCK_STREAM)
    server_socket.bind(("192.168.1.11", 5555))
    ...
    while True:
        ...
        def ipcon(event):
            ...
            if ipcon == 0:
                ...
                vs.daemon = True                    #'True' means it is a front thread,it would close when the mainloop() closes
                vs.start()                            #Thread starts
                ip_send_thread=thread.Thread(target=ip_send)
                ip_send_thread.daemon = True
